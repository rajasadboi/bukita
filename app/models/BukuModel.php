...
class BukuModel
{
    ...
    {
        $this->db->query('SELECT buku.*, kategori.nama FROM ' .
        $this->table . ' JOIN kategori ON buku.kategori_id = kategori.id');
    ...
    {
        $key = $_POST['key'];
        $this->db->query("SELECT buku.*, kategori.nama FROM " . $this->table . " JOIN kategori ON buku.kategori_id = kategori.id WHERE judul LIKE :key");
